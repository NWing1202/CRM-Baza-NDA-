...
Проект прошёл **60+ инкрементальных релизов** (v3.0 → v5.7.1) за время эксплуатации — не одноразовая сдача, а постоянный цикл: фидбек от реального пользователя → приоритизация → релиз → следующий цикл. Часть релизов — прямые хотфиксы по обратной связи заказчика (баги на реальных данных, UX-правки после недели использования), часть — плановые технические пассы (security hardening, декомпозиция разросшихся компонентов). Такая история релизов — свидетельство того, что система реально используется, а не лежит на витрине.
